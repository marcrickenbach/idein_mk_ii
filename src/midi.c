...
#include "midi/private/module_data.h"
...
#define MIDI_NODE   DT_NODELABEL(usart1)
...
#if CONFIG_FKMG_MIDI_NO_OPTIMIZATIONS
#pragma GCC push_options
#pragma GCC optimize ("Og")
#warning "midi.c unoptimized!"
#endif

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(midi, CONFIG_FKMG_MIDI_LOG_LEVEL);

/* *****************************************************************************
 * Structs
 */

static struct midi_module_data midi_md = {0};
/* Convenience accessor to keep name short: md - module data. */
#define md midi_md

const struct device * midi_dev = DEVICE_DT_GET(MIDI_NODE); 
...
static struct MIDI_Instance * sm_ctx_to_instance(struct smf_ctx * p_sm_ctx)
{
    /* Iterate thru the instances. */
    struct MIDI_Instance * p_inst = NULL;
...
static enum MIDI_Err_Id check_listener_cfg_param_for_errors(
        struct MIDI_Listener_Cfg * p_cfg )
{
    if(!p_cfg
    || !p_cfg->p_iface
    || !p_cfg->p_lsnr) return(k_MIDI_Err_Id_Configuration_Invalid);

    /* Is signal valid? */
    if(p_cfg->sig > k_MIDI_Sig_Max)
        return(k_MIDI_Err_Id_Configuration_Invalid);

    /* Is callback valid? */
    if(!p_cfg->cb)
        return(k_MIDI_Err_Id_Configuration_Invalid);

    return(k_MIDI_Err_Id_None);
}
#endif

static void clear_listener(struct MIDI_Listener * p_lsnr)
{
...
static void init_listener(struct MIDI_Listener * p_lsnr)
{
...
#if CONFIG_FKMG_MIDI_RUNTIME_ERROR_CHECKING
static enum MIDI_Err_Id check_instance_cfg_param_for_errors(
        struct MIDI_Instance_Cfg * p_cfg)
{
    if(!p_cfg
    || !p_cfg->p_inst
    || !p_cfg->task.sm.p_thread
    || !p_cfg->task.sm.p_stack
    || !p_cfg->msgq.p_sm_evts) return(k_MIDI_Err_Id_Configuration_Invalid);

    if(p_cfg->task.sm.stack_sz == 0) return(k_MIDI_Err_Id_Configuration_Invalid);
...
static void init_instance_lists(struct MIDI_Instance * p_inst)
{
    for(enum MIDI_Evt_Sig sig = k_MIDI_Evt_Sig_Beg;
                         sig < k_MIDI_Evt_Sig_End;
                         sig++){
        sys_slist_init(&p_inst->list.listeners[sig]);
    }
}

static void clear_instance(struct MIDI_Instance * p_inst)
{
    memset(p_inst, 0, sizeof(*p_inst));
}

static void init_instance(struct MIDI_Instance * p_inst)
{
    clear_instance(p_inst);
    init_instance_lists(p_inst);
    #if CONFIG_FKMG_MIDI_RUNTIME_ERROR_CHECKING
    p_inst->err = k_MIDI_Err_Id_None;
    #endif
}
        ...
        enum MIDI_Err_Id       err )
{
    ...
    return(err != k_MIDI_Err_Id_None);
}
#endif /* CONFIG_FKMG_MIDI_RUNTIME_ERR OR_CHECKING */
        ...
        struct MIDI_Evt      * p_evt)
{
    ...
    enum MIDI_Evt_Sig sig = p_evt->sig;
    struct MIDI_Listener * p_lsnr = NULL;
...
{
    struct MIDI_Evt evt = {
            .sig = k_MIDI_Evt_Sig_Instance_Initialized,
            .data.initd.p_inst = p_inst
    };

    struct MIDI_Listener lsnr = {
        .p_inst = p_inst,
        .cb = cb
    };
...
{
    struct MIDI_Evt evt = {
            .sig = k_MIDI_Instance_Deinitialized,
    ...
    struct MIDI_Listener_Cfg * p_cfg)
{
    ...
    struct MIDI_Instance * p_inst = p_cfg->p_inst;

    /* Get pointer to configured listener. */
    struct MIDI_Listener * p_lsnr = p_cfg->p_lsnr;

    /* Get signal to listen for. */
    enum MIDI_Evt_Sig sig = p_cfg->sig;
...
static void q_sm_event(struct MIDI_Instance * p_inst, struct MIDI_SM_Evt * p_evt)
{
...
static void q_init_instance_event(struct MIDI_Instance_Cfg * p_cfg)
{
    struct MIDI_SM_Evt evt = {
            .sig = k_MIDI_SM_Evt_Sig_Init_Instance,
...
static void state_init_run(void * o)
{
    ...
    struct MIDI_Instance * p_inst = sm_ctx_to_instance(p_sm);

    /* Get the event. */
    struct MIDI_SM_Evt * p_evt = &p_inst->sm_evt;

    /* Expecting only an "init instance" event. Anything else is an error. */
    assert(p_evt->sig == k_MIDI_SM_Evt_Sig_Init_Instance);

    /* We init'd required params on the caller's thread (i.e.
     * MIDI_Init_Instance()), now finish the job. Since this is an
     * Init_Instance event the data contains the intance cfg. */
    struct MIDI_SM_Evt_Sig_Init_Instance * p_ii = &p_evt->data.init_inst;
...
static void state_run_run(void * o)
{
    ...
    switch(p_evt->sig){
        default: break;
        case k_MIDI_SM_Evt_Sig_Init_Instance:
            /* Should never occur. */
            assert(false);
            break;
        #if CONFIG_FKMG_MIDI_SHUTDOWN_ENABLED
        case k_MIDI_Evt_Sig_Instance_Deinitialized:
            assert(false);
            break;
        #endif
    }
...
{
    struct MIDI_Instance * p_inst =
            CONTAINER_OF(item, struct MIDI_Instance, work);
...
static void thread(void * p_1, /* struct MIDI_Instance* */
        void * p_2_unused, void * p_3_unused)
{
    struct MIDI_Instance * p_inst = p_1;
    // printk("MIDI Thread Start. \n"); 
