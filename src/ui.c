/* *****************************************************************************
 * @brief UI implementation.
 */

/* *****************************************************************************
 * TODO
 * Set up GPIO interrupts for UI buttons.
    * BTN_0     - PC13
    * BTN_1     - PB13
    * BTN_2     - PB14
    * BTN_3     - PC7
    * BTN_4     - PC8
    * BTN_5     - PC9
    * ENC_BTN   - PB2

 * Set up GPIO outputs for UI LEDs.
    * LED_BTN_1 - PA15
    * LED_BTN_2 - PC11
    * LED_BTN_3 - PC12
    * LED_BTN_4 - PD2
    * LED_BTN_5 - PB3
    * LED_MODE_BTN_RED - PC14
    * LED_MODE_BTN_GRN - PC15

 * Set up Timer for Encoder.
    * TIM2_CH1 - PA0
    * TIM2_CH2 - PA1
 */

/* *****************************************************************************
 * Includes
 */

#include "ui.h"

#include <zephyr/smf.h>
#include <zephyr/device.h>

#include <zephyr/drivers/gpio.h>

#include <assert.h>

#include "ui/private/sm_evt.h"
#include "ui/private/module_data.h"

/* *****************************************************************************
 * Constants, Defines, and Macros
 */

#define SUCCESS (0)
#define FAIL    (-1)

#define OVERRIDE    true
#define NO_OVERRIDE false

#define GPIO_PINS   DT_PATH(zephyr_user)

/* *****************************************************************************
 * Debugging
 */

#if CONFIG_FKMG_UI_NO_OPTIMIZATIONS
#pragma GCC push_options
#pragma GCC optimize ("Og")
#warning "ui.c unoptimized!"
#endif

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(ui, CONFIG_FKMG_UI_LOG_LEVEL);

/* *****************************************************************************
 * Structs
 */

static struct ui_module_data ui_md = {0};
/* Convenience accessor to keep name short: md - module data. */
#define md ui_md

struct gpio_callback button_0_gpio_cb;
struct gpio_callback button_1_gpio_cb;
struct gpio_callback button_2_gpio_cb;
struct gpio_callback button_3_gpio_cb;
struct gpio_callback button_4_gpio_cb;
struct gpio_callback button_5_gpio_cb;
struct gpio_callback button_encoder_gpio_cb;

static const struct gpio_dt_spec button_0       = GPIO_DT_SPEC_GET(GPIO_PINS, btn0_gpios);
static const struct gpio_dt_spec button_1       = GPIO_DT_SPEC_GET(GPIO_PINS, btn1_gpios);
static const struct gpio_dt_spec button_2       = GPIO_DT_SPEC_GET(GPIO_PINS, btn2_gpios);
static const struct gpio_dt_spec button_3       = GPIO_DT_SPEC_GET(GPIO_PINS, btn3_gpios);
static const struct gpio_dt_spec button_4       = GPIO_DT_SPEC_GET(GPIO_PINS, btn4_gpios);
static const struct gpio_dt_spec button_5       = GPIO_DT_SPEC_GET(GPIO_PINS, btn5_gpios);
static const struct gpio_dt_spec button_encoder = GPIO_DT_SPEC_GET(GPIO_PINS, mode_gpios);

static const struct gpio_dt_spec led_0_btn          = GPIO_DT_SPEC_GET(GPIO_PINS, led_0_gpios);
static const struct gpio_dt_spec led_1_btn          = GPIO_DT_SPEC_GET(GPIO_PINS, led_1_gpios);
static const struct gpio_dt_spec led_2_btn          = GPIO_DT_SPEC_GET(GPIO_PINS, led_2_gpios);
static const struct gpio_dt_spec led_3_btn          = GPIO_DT_SPEC_GET(GPIO_PINS, led_3_gpios);
static const struct gpio_dt_spec led_4_btn          = GPIO_DT_SPEC_GET(GPIO_PINS, led_4_gpios);
static const struct gpio_dt_spec led_mode_red_btn   = GPIO_DT_SPEC_GET(GPIO_PINS, led_modered_gpios);
static const struct gpio_dt_spec led_mode_green_btn = GPIO_DT_SPEC_GET(GPIO_PINS, led_modegreen_gpios);
static const struct gpio_dt_spec led_enc_btn_1      = GPIO_DT_SPEC_GET(GPIO_PINS, enc_led_1_gpios);
static const struct gpio_dt_spec led_enc_btn_2      = GPIO_DT_SPEC_GET(GPIO_PINS, enc_led_2_gpios);


static void on_button_0(const struct device *dev, struct gpio_callback *cb, uint32_t pins);
static void on_button_1(const struct device *dev, struct gpio_callback *cb, uint32_t pins);
static void on_button_2(const struct device *dev, struct gpio_callback *cb, uint32_t pins);
static void on_button_3(const struct device *dev, struct gpio_callback *cb, uint32_t pins);
static void on_button_4(const struct device *dev, struct gpio_callback *cb, uint32_t pins);
static void on_button_5(const struct device *dev, struct gpio_callback *cb, uint32_t pins);
static void on_button_encoder(const struct device *dev, struct gpio_callback *cb, uint32_t pins);



/* *****************************************************************************
 * Private
 */

/* *****
 * Utils
 * *****/

static bool instance_contains_state_machine_ctx(
        struct UI_Instance * p_inst,
        struct smf_ctx      * p_sm_ctx)
{
    return(&p_inst->sm == p_sm_ctx);
}

/* Find the instance that contains the state machine context. */
static struct UI_Instance * sm_ctx_to_instance(struct smf_ctx * p_sm_ctx)
{
    /* Iterate thru the instances. */
    struct UI_Instance * p_inst = NULL;
    SYS_SLIST_FOR_EACH_CONTAINER(&md.list.instances, p_inst, node.instance){
        if(instance_contains_state_machine_ctx(p_inst, p_sm_ctx)) return p_inst;
    }
    return(NULL);
}



/* **************
 * Listener Utils
 * **************/

#if CONFIG_FKMG_UI_RUNTIME_ERROR_CHECKING
static enum UI_Err_Id check_listener_cfg_param_for_errors(
        struct UI_Listener_Cfg * p_cfg )
{
    if(!p_cfg
    || !p_cfg->p_iface
    || !p_cfg->p_lsnr) return(k_UI_Err_Id_Configuration_Invalid);

    /* Is signal valid? */
    if(p_cfg->sig > k_UI_Sig_Max)
        return(k_UI_Err_Id_Configuration_Invalid);

    /* Is callback valid? */
    if(!p_cfg->cb)
        return(k_UI_Err_Id_Configuration_Invalid);

    return(k_UI_Err_Id_None);
}
#endif

static void clear_listener(struct UI_Listener * p_lsnr)
{
    memset(p_lsnr, 0, sizeof(*p_lsnr));
}

static void config_listener(
        struct UI_Listener     * p_lsnr,
        struct UI_Listener_Cfg * p_cfg)
{
    /* Set listner's instance it is listening to. */
    p_lsnr->p_inst = p_cfg->p_inst;

    /* Set listner's callback. */ 
    p_lsnr->cb = p_cfg->cb;
}

static void init_listener(struct UI_Listener * p_lsnr)
{
    clear_listener(p_lsnr);
}

/* **************
 * Instance Utils
 * **************/

#if CONFIG_FKMG_UI_RUNTIME_ERROR_CHECKING
static enum UI_Err_Id check_instance_cfg_param_for_errors(
        struct UI_Instance_Cfg * p_cfg)
{
    if(!p_cfg
    || !p_cfg->p_inst
    || !p_cfg->task.sm.p_thread
    || !p_cfg->task.sm.p_stack
    || !p_cfg->msgq.p_sm_evts) return(k_UI_Err_Id_Configuration_Invalid);

    if(p_cfg->task.sm.stack_sz == 0) return(k_UI_Err_Id_Configuration_Invalid);

    return(k_UI_Err_Id_None);
}
#endif

static void add_instance_to_instances(
        struct UI_Instance  * p_inst)
{
    sys_slist_append(&md.list.instances, &p_inst->node.instance);
}

static void config_instance_queues(
        struct UI_Instance     * p_inst,
        struct UI_Instance_Cfg * p_cfg)
{
    p_inst->msgq.p_sm_evts = p_cfg->msgq.p_sm_evts;
}

/* Forward reference */
static void button_gpio_init(struct UI_Instance * p_inst); 

static void config_instance_deferred(
        struct Pot_Instance     * p_inst,
        struct Pot_Instance_Cfg * p_cfg)
{
    button_gpio_init(p_inst);
}

/* Since configuration starts on caller's thread, configure fields that require
 * immediate and/or inconsequential configuration and defer rest to be handled
 * by our own thread later. */
static void config_instance_immediate(
        struct UI_Instance     * p_inst,
        struct UI_Instance_Cfg * p_cfg)
{
    config_instance_queues(p_inst, p_cfg);
}

static void init_instance_lists(struct UI_Instance * p_inst)
{
    for(enum UI_Evt_Sig sig = k_UI_Evt_Sig_Beg;
                         sig < k_UI_Evt_Sig_End;
                         sig++){
        sys_slist_init(&p_inst->list.listeners[sig]);
    }
}

static void clear_instance(struct UI_Instance * p_inst)
{
    memset(p_inst, 0, sizeof(*p_inst));
}

static void init_instance(struct UI_Instance * p_inst)
{
    clear_instance(p_inst);
    init_instance_lists(p_inst);
    #if CONFIG_FKMG_UI_RUNTIME_ERROR_CHECKING
    p_inst->err = k_UI_Err_Id_None;
    #endif
}

/* ************
 * Module Utils
 * ************/

static void init_module_lists(void)
{
    sys_slist_init(&md.list.instances);
}

static void clear_module(void)
{
    memset(&md, 0, sizeof(md));
}

static void init_module(void)
{
    if(md.initialized) return;
    clear_module();
    init_module_lists();
    md.initialized = true;
}

/* **************
 * Error Checking
 * **************/

#if CONFIG_FKMG_UI_RUNTIME_ERROR_CHECKING
static void set_error(
        struct UI_Instance * p_inst,
        enum UI_Err_Id       err,
        bool                  override)
{
    if(p_inst){
        if((override                          )
        || (p_inst->err == k_UI_Err_Id_None )){
            p_inst->err = err;
        }
    }
}

static bool errored(
        struct UI_Instance * p_inst,
        enum UI_Err_Id       err )
{
    set_error(p_inst, err, NO_OVERRIDE);
    return(err != k_UI_Err_Id_None);
}
#endif /* CONFIG_FKMG_UI_RUNTIME_ERROR_CHECKING */

/* ************
 * Broadcasting
 * ************/

static void broadcast(
        struct UI_Evt      * p_evt,
        struct UI_Listener * p_lsnr)
{
    /* call the listener, passing the event */
    if(p_lsnr->cb) p_lsnr->cb(p_evt);
}

static void broadcast_event_to_listeners(
        struct UI_Instance * p_inst,
        struct UI_Evt      * p_evt)
{
    enum UI_Evt_Sig sig = p_evt->sig;
    struct UI_Listener * p_lsnr = NULL;
    SYS_SLIST_FOR_EACH_CONTAINER(&p_inst->list.listeners[sig], p_lsnr, node.listener){
        broadcast(p_evt, p_lsnr);
    }
}

/* There's only 1 listener for instance initialization, and it is provided in
 * the cfg struct. */
static void broadcast_instance_initialized(
        struct UI_Instance * p_inst,
        UI_Listener_Cb       cb)
{
    struct UI_Evt evt = {
            .sig = k_UI_Evt_Sig_Instance_Initialized,
            .data.initd.p_inst = p_inst
    };

    struct UI_Listener lsnr = {
        .p_inst = p_inst,
        .cb = cb
    };

    broadcast(&evt, &lsnr);
}

#if CONFIG_FKMG_UI_ALLOW_SHUTDOWN
static void broadcast_interface_deinitialized(
        struct UI_Instance * p_inst,
        UI_Listener_Cb       cb)
{
    struct UI_Evt evt = {
            .sig = k_UI_Instance_Deinitialized,
            .data.inst_deinit.p_inst = p_inst
    };

    struct UI_Listener lsnr = {
        .p_inst = p_inst,
        .cb = cb
    };

    broadcast(&evt, &lsnr);
}
#endif


/* **************
 * Listener Utils
 * **************/

static void add_listener_for_signal_to_listener_list(
    struct UI_Listener_Cfg * p_cfg)
{
    /* Get pointer to interface to add listener to. */
    struct UI_Instance * p_inst = p_cfg->p_inst;

    /* Get pointer to configured listener. */
    struct UI_Listener * p_lsnr = p_cfg->p_lsnr;

    /* Get signal to listen for. */
    enum UI_Evt_Sig sig = p_cfg->sig;

    /* Add listener to instance's specified signal. */
    sys_slist_t * p_list = &p_inst->list.listeners[sig];
    sys_snode_t * p_node = &p_lsnr->node.listener;
    sys_slist_append(p_list, p_node);
}

#if CONFIG_FKMG_UI_ALLOW_LISTENER_REMOVAL
static bool find_list_containing_listener_and_remove_listener(
    struct UI_Instance * p_inst,
	struct UI_Listener * p_lsnr)
{
    for(enum UI_Evt_Sig sig = k_UI_Evt_Sig_Beg;
                         sig < k_UI_Evt_Sig_End;
                         sig++){
        bool found_and_removed = sys_slist_find_and_remove(
                &p_inst->list.listeners[sig], &p_lsnr->node);
        if(found_and_removed) return(true);
    }
    return( false );
}
#endif



/* **************
 * Event Queueing
 * **************/

static void q_sm_event(
        struct UI_Instance * p_inst, struct UI_SM_Evt * p_evt)
{
    bool queued = k_msgq_put(p_inst->msgq.p_sm_evts, p_evt, K_NO_WAIT) == 0;

    if(!queued) assert(false);
}

static void q_init_instance_event(struct UI_Instance_Cfg * p_cfg)
{
    struct UI_SM_Evt evt = {
            .sig = k_UI_SM_Evt_Sig_Init_Instance,
            .data.init_inst.cfg = *p_cfg
    };
    struct UI_Instance * p_inst = p_cfg->p_inst;
    q_sm_event(p_inst, &evt);
}


static void button_gpio_pin_config(void) {
    
    int ret;
    
    gpio_pin_configure(button_0.port, button_0.pin, GPIO_INPUT | GPIO_PULL_DOWN);
    gpio_pin_configure(button_1.port, button_1.pin, GPIO_INPUT | GPIO_PULL_DOWN);
    gpio_pin_configure(button_2.port, button_2.pin, GPIO_INPUT | GPIO_PULL_DOWN);
    gpio_pin_configure(button_3.port, button_3.pin, GPIO_INPUT | GPIO_PULL_DOWN);
    gpio_pin_configure(button_4.port, button_4.pin, GPIO_INPUT | GPIO_PULL_DOWN);
    gpio_pin_configure(button_5.port, button_5.pin, GPIO_INPUT | GPIO_PULL_DOWN);
    gpio_pin_configure(button_encoder.port, button_encoder.pin, GPIO_INPUT | GPIO_PULL_DOWN);

    if (!device_is_ready(led_0_btn.port)
        ||  !device_is_ready(led_1_btn.port)
        ||  !device_is_ready(led_2_btn.port)
        ||  !device_is_ready(led_3_btn.port)
        ||  !device_is_ready(led_4_btn.port)
        ||  !device_is_ready(led_mode_red_btn.port)
        ||  !device_is_ready(led_mode_green_btn.port)
        ||  !device_is_ready(led_enc_btn_1.port)
        ||  !device_is_ready(led_enc_btn_2.port)) {
        ret = 1;
    }

    gpio_pin_configure_dt(&led_0_btn, GPIO_OUTPUT_LOW);
    gpio_pin_configure_dt(&led_1_btn, GPIO_OUTPUT_LOW);
    gpio_pin_configure_dt(&led_2_btn, GPIO_OUTPUT_LOW);
    gpio_pin_configure_dt(&led_3_btn, GPIO_OUTPUT_LOW);
    gpio_pin_configure_dt(&led_4_btn, GPIO_OUTPUT_LOW);
    gpio_pin_configure_dt(&led_mode_red_btn, GPIO_OUTPUT_LOW);
    gpio_pin_configure_dt(&led_mode_green_btn, GPIO_OUTPUT_LOW);
    gpio_pin_configure_dt(&led_enc_btn_1, GPIO_OUTPUT_LOW);

    ret = gpio_pin_configure_dt(&led_enc_btn_2, GPIO_OUTPUT_LOW);
    
    if (ret < 0) {
        LOG_ERR("Error: Failed to configure LED pin");
    }
}

static void button_gpio_interrupt_config(void)
{
    int ret;
    
    gpio_pin_interrupt_configure(button_0.port, button_0.pin, GPIO_INT_EDGE_BOTH);
    gpio_pin_interrupt_configure(button_1.port, button_1.pin, GPIO_INT_EDGE_BOTH);
    gpio_pin_interrupt_configure(button_2.port, button_2.pin, GPIO_INT_EDGE_BOTH);
    gpio_pin_interrupt_configure(button_3.port, button_3.pin, GPIO_INT_EDGE_BOTH);
    gpio_pin_interrupt_configure(button_4.port, button_4.pin, GPIO_INT_EDGE_BOTH);
    gpio_pin_interrupt_configure(button_5.port, button_5.pin, GPIO_INT_EDGE_BOTH);

    ret = gpio_pin_interrupt_configure(button_encoder.port, button_encoder.pin, GPIO_INT_EDGE_BOTH);
    
    if (ret < 0) {
        LOG_ERR("Error: Failed to configure interrupt on resetB pin");
    }

}


static void button_gpio_callback_config(struct UI_Instance * p_inst)
{
    int ret; 

    gpio_init_callback(&button_0_gpio_cb, on_button_0, BIT(button_0.pin));
    gpio_init_callback(&button_1_gpio_cb, on_button_1, BIT(button_1.pin));
    gpio_init_callback(&button_2_gpio_cb, on_button_2, BIT(button_2.pin));
    gpio_init_callback(&button_3_gpio_cb, on_button_3, BIT(button_3.pin));
    gpio_init_callback(&button_4_gpio_cb, on_button_4, BIT(button_4.pin));
    gpio_init_callback(&button_5_gpio_cb, on_button_5, BIT(button_5.pin));
    gpio_init_callback(&button_encoder_gpio_cb, on_button_encoder, BIT(button_encoder.pin));

    gpio_add_callback(button_0.port, &button_0_gpio_cb);
    gpio_add_callback(button_1.port, &button_1_gpio_cb);
    gpio_add_callback(button_2.port, &button_2_gpio_cb);
    gpio_add_callback(button_3.port, &button_3_gpio_cb);
    gpio_add_callback(button_4.port, &button_4_gpio_cb);
    gpio_add_callback(button_5.port, &button_5_gpio_cb);

    ret = gpio_add_callback(button_encoder.port, &button_encoder_gpio_cb);

    if (ret < 0) {
        LOG_ERR("Error: Failed to add callback on resetB pin");
    }
}


static void button_gpio_init(struct UI_Instance * p_inst) {
    
    int ret = 0; 

    if (!   device_is_ready(button_0.port)
        || !device_is_ready(button_1.port)
        || !device_is_ready(button_2.port)
        || !device_is_ready(button_3.port)
        || !device_is_ready(button_4.port)
        || !device_is_ready(button_5.port)
        || !device_is_ready(button_encoder.port)) {
        ret = 1;
    }

    assert(ret == 0);
    button_gpio_pin_config();
    button_gpio_interrupt_config();
    button_gpio_callback_config(p_inst);

}


/* ********************
 * Button Callbacks
 * ********************/

static void on_button_0(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    LOG_INF("Button 0 pressed");
    
}

static void on_button_1(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    LOG_INF("Button 1 pressed");
}

static void on_button_2(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    LOG_INF("Button 2 pressed");
}

static void on_button_3(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    LOG_INF("Button 3 pressed");
}

static void on_button_4(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    LOG_INF("Button 4 pressed");
}

static void on_button_5(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    LOG_INF("Button 5 pressed");
}

static void on_button_encoder(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    LOG_INF("Button encoder pressed");
}



static void setup_led_indicator(struct UI_Instance * p_inst, bool pass) 
{
    if (pass) {

        for (int i = 0; i < 6; i++) {
            gpio_pin_set(led_0_btn.port, led_0_btn.pin, 1);
            k_msleep(500); 
            gpio_pin_set(led_0_btn.port, led_0_btn.pin, 0);
            k_msleep(500); 
        }

    } else {
        for (int i = 0; i < 6; i++) {
            gpio_pin_set(led_4_btn.port, led_4_btn.pin, 1);
            k_msleep(500); 
            gpio_pin_set(led_4_btn.port, led_4_btn.pin, 0);
            k_msleep(500); 
        }
    }
}

/* **********
 * FSM States
 * **********/

/* The states are flat:
 *
 *  State init:
 *
 *  |entry          |  |run                |  |exit           |
 *  |---------------|  |-------------------|  |---------------|
 *  |not implemented|  |init instance:     |  |not implemented|
 *                     |* finish cfg.      |
 *                     |* ->run            |
 *                     |all other signals  |
 *                     |not allowed; assert|
 *
 *  State run:
 *
 *  |entry             |  |run             |  |exit           |
 *  |------------------|  |----------------|  |---------------|
 *  |not implemented   |  |convert:        |  |not implemented|
 *  |                  |  |* UI calculate |
 *                        |* quantize cv   |
 *                        |* filter        |
 *                        |deinit:         |
 *                        |* ->deinit      |
 *                        |all others:     |
 *                        |* assert        |
 *
 *  State deinit:
 *
 *  |entry             |  |run                 |  |exit           |
 *  |------------------|  |--------------------|  |---------------|
 *  |* start conversion|  |* finish dcfg.      |  |not implemented|
 *  |  timer           |  |* exit state machine|
 *                        |all others:         |
 *                        |* assert            |
 *
 *  Legend:
 *  "entry state": 1st state of state machine
 *  "xxx:"       : signal to handle
 *  "*"          : action to take on signal
 *  "-> xxx"     : state to change to
 *  "^ xxx"      : pass signal to parent, with expected parent action on signal.
 */


/* Forward declaration of state table */
static const struct smf_state states[];

enum state{
    init,   // Init instance - should only occur once, after thread start
    run,    // Run - handles all events while running
    deinit, // Deinit instance - should only occur once, after deinit event
            // (if implemented)
};



/* NOTE: in all the state functions the param o is a pointer to the state
 * machine context, which is &p_inst->sm. */

/* Init state responsibility is to complete initialization of the instance, let
 * an instance config listener know we've completed initialization (i.e. thread
 * is up and running), and then transition to next state. Init state occurs
 * immediately after thread start and is expected to only occur once. */

static void state_init_run(void * o)
{
    struct smf_ctx * p_sm = o;
    struct UI_Instance * p_inst = sm_ctx_to_instance(p_sm);

    /* Get the event. */
    struct UI_SM_Evt * p_evt = &p_inst->sm_evt;

    /* Expecting only an "init instance" event. Anything else is an error. */
    assert(p_evt->sig == k_UI_SM_Evt_Sig_Init_Instance);

    /* We init'd required params on the caller's thread (i.e.
     * UI_Init_Instance()), now finish the job. Since this is an
     * Init_Instance event the data contains the intance cfg. */
    struct UI_SM_Evt_Sig_Init_Instance * p_ii = &p_evt->data.init_inst;

    broadcast_instance_initialized(p_inst, p_ii->cfg.cb);

    config_instance_deferred(p_inst, &p_ii->cfg);

    smf_set_state(SMF_CTX(p_sm), &states[run]);
}

/* Run state responsibility is to be the root state to handle everything else
 * other than instance initialization. */

static void state_run_entry(void * o)
{
    struct smf_ctx * p_sm = o;
    struct UI_Instance * p_inst = sm_ctx_to_instance(p_sm);


}

static void state_run_run(void * o)
{
    struct smf_ctx * p_sm = o;
    struct UI_Instance * p_inst = sm_ctx_to_instance(p_sm);

    /* Get the event. */
    struct UI_SM_Evt * p_evt = &p_inst->sm_evt;

    switch(p_evt->sig){
        default: break;
        case k_UI_SM_Evt_Sig_Init_Instance:
            /* Should never occur. */
            assert(false);
            break;
        case k_UI_Evt_Sig_Write:

            break;
        case k_UI_SM_Evt_Sig_Sensor_Setup_Complete:
            bool result = p_evt->data.pass.pass;
            setup_led_indicator(p_inst, result);
            
            break;
        #if CONFIG_FKMG_UI_SHUTDOWN_ENABLED
        case k_UI_Evt_Sig_Instance_Deinitialized:
            assert(false);
            break;
        #endif
    }
}

/* Deinit state responsibility is to clean up before exiting thread. */
#if CONFIG_FKMG_UI_SHUTDOWN_ENABLED
static void state_deinit_run(void * o)
{
    struct smf_ctx * p_sm = o;
    struct UI_Instance * p_inst = sm_ctx_to_instance(p_sm);

    /* Get the event. */
    struct UI_SM_Evt * p_evt = &p_inst->sm_evt;

    /* TODO */
}
#endif

static const struct smf_state states[] = {
    /*                                      entry               run  exit */
    [  init] = SMF_CREATE_STATE(           NULL,   state_init_run, NULL),
    [   run] = SMF_CREATE_STATE(state_run_entry,    state_run_run, NULL),
    #if CONFIG_FKMG_UI_SHUTDOWN_ENABLED
    [deinit] = SMF_CREATE_STATE(           NULL, state_deinit_run, NULL),
    #endif
};

/* ******
 * Thread
 * ******/

#if CONFIG_FKMG_UI_ALLOW_SHUTDOWN
/* Since there is only the "join" facility to know when a thread is shut down,
 * and that isn't appropriate to use since it will put the calling thread to
 * sleep until the other thread is shut down, we set up a delayable system work
 * queue event to check that the thread is shut down and then call any callback
 * that is waiting to be notified. */
void on_thread_shutdown(struct k_work *item)
{
    struct UI_Instance * p_inst =
            CONTAINER_OF(item, struct UI_Instance, work);

    char * thread_state_str = "dead";
    k_thread_state_str(&p_inst->thread, thread_state_str, sizeof(thread_state_str));
    bool shut_down = strcmp( thread_state_str, "dead" ) == 0;

    if(!shut_down) k_work_reschedule(&p_inst->work, K_MSEC(1));
    else broadcast_instance_deinitialized(p_inst);
}
#endif

static void thread(void * p_1, /* struct UI_Instance* */
        void * p_2_unused, void * p_3_unused)
{
    struct UI_Instance * p_inst = p_1;
    /* NOTE: smf_set_initial() executes the entry state. */
    struct smf_ctx * p_sm = &p_inst->sm;
    smf_set_initial(SMF_CTX(p_sm), &states[init]);

    /* Get the state machine event queue and point to where to put the dequeued
     * event. */
    struct k_msgq * p_msgq = p_inst->msgq.p_sm_evts;
    struct UI_SM_Evt * p_evt = &p_inst->sm_evt;

    bool run = true;

    while(run){
        /* Wait on state machine event. Cache it then run state machine. */
        k_msgq_get(p_msgq, p_evt, K_FOREVER);
        run = smf_run_state(SMF_CTX(p_sm)) == 0;
    }

    #if CONFIG_FKMG_UI_ALLOW_SHUTDOWN
    /* We're shutting down. Schedule a work queue event to check that the
     * thread exited and call back anything. */
    if(should_callback_on_exit(p_inst)){
        k_work_init_delayable( &p_inst->work, on_thread_shutdown);
        k_work_schedule(&p_inst->work, K_MSEC(1));
    }
    #endif
}

static void start_thread(
        struct UI_Instance     * p_inst,
        struct UI_Instance_Cfg * p_cfg)
{
    struct k_thread  * p_thread = p_cfg->task.sm.p_thread;
    k_thread_stack_t * p_stack  = p_cfg->task.sm.p_stack;
    size_t stack_sz_bytes = p_cfg->task.sm.stack_sz;
    int prio = p_cfg->task.sm.prio;

    /* Start the state machine thread. */
    p_inst->task.sm.p_thread = k_thread_create(
        p_thread,       // ptr to uninitialized struct k_thread
        p_stack,        // ptr to the stack space
        stack_sz_bytes, // stack size in bytes
        thread,         // thread entry function
        p_inst,         // 1st entry point parameter
        NULL,           // 2nd entry point parameter
        NULL,           // 3rd entry point parameter
        prio,           // thread priority 
        0,              // thread options
        K_NO_WAIT);     // scheduling delay
    assert(p_inst->task.sm.p_thread == p_thread);
}

/* *****************************************************************************
 * Public
 */

void UI_Init_Instance(struct UI_Instance_Cfg * p_cfg)
{
    /* Get pointer to instance to configure. */
    struct UI_Instance * p_inst = p_cfg->p_inst;

    #if CONFIG_FKMG_UI_RUNTIME_ERROR_CHECKING
    /* Check instance configuration for errors. */
    if(errored(p_inst, check_instance_cfg_param_for_errors(p_cfg))){
        assert(false);
        return;
    }
    #endif


    init_module();
    init_instance(p_inst);
    config_instance_immediate(p_inst, p_cfg);
    add_instance_to_instances(p_inst);
    start_thread(p_inst, p_cfg);
    q_init_instance_event(p_cfg);

}

#if CONFIG_FKMG_UI_ALLOW_SHUTDOWN
void UI_Deinit_Instance(struct UI_Instance_Dcfg * p_dcfg)
{
    #error "Not implemented yet!"
}
#endif

void UI_Add_Listener(struct UI_Listener_Cfg * p_cfg)
{
    #if CONFIG_FKMG_UI_RUNTIME_ERROR_CHECKING
    /* Get pointer to instance. */
    struct UI_Instance * p_inst = p_cfg->p_inst;

    /* Check listener instance configuration for errors. */
    if(errored(p_inst, check_listener_cfg_param_for_errors(p_cfg))){
        assert(false);
        return;
    }
    #endif

    struct UI_Listener * p_lsnr = p_cfg->p_lsnr;
    init_listener(p_lsnr);
    config_listener(p_lsnr, p_cfg);
    add_listener_for_signal_to_listener_list(p_cfg);
}


#if CONFIG_FKMG_UI_ALLOW_LISTENER_REMOVAL
void UI_Remove_Listener(struct UI_Listener * p_lsnr)
{
    #error "Not implemented yet!"
}
#endif

#if CONFIG_FKMG_UI_NO_OPTIMIZATIONS
#pragma GCC pop_options
#endif
